package com.derbysoft.nuke.dlm.server.log;
...
public class LogbackInitializing {
    ...
    public LogbackInitializing(PropertySources properties) {
        try {
    ...
    private Appender<ILoggingEvent> createAppender(String name, PropertyResolver resolver, LoggerContext loggerContext) {
    ...
    private RollingFileAppender<ILoggingEvent> createFileAppender(String name, PropertyResolver resolver, LoggerContext loggerContext) {
        ...
        if (!resolver.containsProperty("log." + name + ".history.max")) {
            ...
            timeBasedRollingPolicy.setMaxHistory(Integer.parseInt(resolver.getProperty("log." + name + ".history.max")));
        }
